...
db["players"] = JSON.parse(fs.readFileSync("server/data/jogadores.json"))[
  "players"
];
db["gamesForPlayers"] = JSON.parse(
  fs.readFileSync("server/data/jogosPorJogador.json")
);
...
app.get("/jogador/:numero_identificador/", (req, res) => {
  ...
  const gamesForPlayer =
    db["gamesForPlayers"][req.params.numero_identificador]["games"];
  ...
  const gamesCount =
    db["gamesForPlayers"][req.params.numero_identificador]["game_count"];
  const unplayedGames = _.countBy(gamesForPlayer, (el) => {
